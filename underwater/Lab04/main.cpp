...
GLuint sphereVAO;
std::vector<float> vertices; // ���ڴ洢����Ķ�������




float radians(float degrees) {
	return degrees * (M_PI / 180.0f); // M_PI ��Բ���ʣ�180.0f �Ƕ���ת��Ϊ���ȵ�����
}
...
float squidSpeed = 0.5f; // �����ƶ��ٶ�
float sharkSpeed = 1.8f; // �����ƶ��ٶ�
float seahorseDirection = 1.0f; // ���㷽��1:���ϣ�-1:���£�
float sharkDirectionX = 1.0f; // ����ˮƽ�ƶ�����
float sharkRotationY = 0.0f;
float squidDirectionX = 1.1f; // ����ˮƽ�ƶ�����

float aincradRotationX = 0.0f; // ���ڴ洢aincrad.dae����ת�Ƕ�
...
void generateSphere(float radius, int sectors, int stacks, std::vector<float>& vertices) {
	for (int i = 0; i <= stacks; ++i) {
		float stackAngle = M_PI / 2 - i * M_PI / stacks; // �Ӧ�/2��-��/2
		float xy = radius * cosf(stackAngle); // �뾶ͶӰ
		float z = radius * sinf(stackAngle); // z����

		for (int j = 0; j <= sectors; ++j) {
			float sectorAngle = j * 2 * M_PI / sectors; // �Ƕ�
...
void initSphere() {
	// �������嶥��
	generateSphere(1.0f, 36, 18, vertices); // �뾶 1.0��36 ��������18 ����ջ
	...
	// ���ö�������ָ��
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0); // λ��
...
void display() {
	...
	// ���û�����
	// ���û�����
	glUniform3f(glGetUniformLocation(shaders["model"], "ambientLight"), 0.2f, 0.2f, 0.2f); // ���ӻ�����


	// ����������������
	glUniform3f(glGetUniformLocation(shaders["model"], "Kd"), 1.0f, 0.5f, 0.31f); // �������������ɫ
	glUniform3f(glGetUniformLocation(shaders["model"], "Ks"), 1.0f, 1.0f, 1.0f); // ���淴����ɫ
	glUniform1f(glGetUniformLocation(shaders["model"], "Shininess"), 32.0f); // �߹�ǿ��

	// ���ù�Դλ�ú�ǿ��
	glUniform4f(glGetUniformLocation(shaders["model"], "LightPosition"), 0.0f, 5.0f, 0.0f, 1.0f); // ��Դ�ڿ���
	//glUniform3f(glGetUniformLocation(shaders["model"], "Ld"), 1.0f, 1.0f, 1.0f); // ��Դ��ɫ����ɫ��
	updateLightUniforms();
	glUniform1i(glGetUniformLocation(shaders["model"], "useOriginShader"), useOriginShader); // ��Դ�ڿ���
	...
	for (const auto& model : models) {
		...
		if ("assets/shark3.dae" == model.name) {
			...
			modelMatrix = rotate_y_deg(modelMatrix, aincradRotationX); // Ӧ��Y����ת
		}
		else {
	...
	view = translate(view, vec3(0.0f, 0.0f, -5.0f)); // �ʵ��������λ��
...
void updateScene() {
	...
	// ����aincrad����ת�Ƕ�
	aincradRotationX += 10.0f * delta; // ÿ����ת10�ȣ����Ը�����Ҫ�����ٶȣ�
	...
	for (auto& fish : models) {
		if (fish.name == "assets/seahorse.dae") { // ��������ģ������Ϊ" squid"
			//fish.position.v[1] += squidSpeed * seahorseDirection * delta; // �����ƶ�
			if (fish.position.v[1] >= -3.0f || fish.position.v[1] <= 3.0f) { // �趨���±߽�
				fish.position.v[1] = 1.0f;
				seahorseDirection *= -1; // ��ת����
			}
			// ���Ӷ���Ч��
			fish.position.v[1] += (rand() % 10 - 5) * 0.03f * seahorseDirection; // С��Χ����
		}

		// ���������X����
		if (fish.name == "assets/shark3.dae") { // ��������ģ������Ϊ"shark"
			fish.position.v[0] += sharkSpeed * sharkDirectionX * delta; // ˮƽ�ƶ�
			if (fish.position.v[0] >= 10.0f || fish.position.v[0] <= -1.0f) { // �趨ˮƽ�߽�
				sharkDirectionX *= -1; // ��ת����
				sharkRotationY += 180.0f; // �ı���ת�Ƕ�
				// �����������ת

			}
		}
		if (fish.name == "assets/squid.dae") { // ��������ģ������Ϊ"shark"
			fish.position.v[0] += squidSpeed * squidDirectionX * delta; // ˮƽ�ƶ�
			if (fish.position.v[0] >= 15.0f || fish.position.v[0] <= -25.0f) { // �趨ˮƽ�߽�
				squidDirectionX *= -1; // ��ת����
			}
		}
	}
...
void keypress(unsigned char key, int x, int y) {
	...
	case 'm':
		// �л���Դ
		lightEnabled = !lightEnabled; // �л���Դ����״̬
		if (lightEnabled) {
			// �����Դ�������л�����һ����Դ����
			currentLightType = (currentLightType == LIGHT_TYPE_1) ? LIGHT_TYPE_2 : LIGHT_TYPE_1;
			currentColorIndex = (currentColorIndex + 1) % 6; // �л�����һ����ɫ
		}
		updateLightUniforms(); // ���¹�Դ�� uniform ����
	case 'n':
